                      ...
                      <CardContent className="p-3 sm:p-4">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          <Avatar className="w-6 h-6">
                            <AvatarFallback className="bg-primary text-white text-[10px] font-[Manrope] font-bold">
                              {revision.author.initials}
                            </AvatarFallback>
                          </Avatar>
                          <span className="font-[Manrope] text-xs sm:text-sm font-semibold text-foreground">
                            {revision.author.name}
                          </span>
                        </div>

                        <div className="flex items-center gap-1.5 text-foreground/60 dark:text-muted-foreground mb-2">
                        ...
                        {revision.changeSummary && (
                          <p className="font-[Manrope] text-xs text-foreground/70 dark:text-muted-foreground line-clamp-2">
                            {revision.changeSummary}
                          </p>
                        )}
            ...
            {selectedRevision ? (
                ...
                <Card>
                  <CardContent className="p-4 sm:p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="font-[Manrope] text-foreground font-bold text-lg sm:text-xl mb-2">
                          Sürüm {selectedRevision.version}
                        </h3>
                        <div className="flex items-center gap-4 text-sm text-foreground/60 dark:text-muted-foreground">
                    ...
                    {viewMode === "content" && (
                      <div className="bg-accent rounded-xl p-4">
                        <p className="font-[Manrope] text-foreground leading-relaxed text-sm sm:text-base whitespace-pre-wrap">
                    ...
                    {viewMode === "diff" && diff && compareRevision && (
                      <div className="bg-accent rounded-xl p-4 overflow-x-auto">
                        <div className="mb-2 flex items-center gap-4 text-xs font-[Manrope] text-foreground/60 dark:text-muted-foreground">
                          <span>
                        ...
                        <div className="font-mono text-xs sm:text-sm">
                          {diff.map((line, index) => (
                            ...
                            >
                              <span className="inline-block w-8 text-right mr-2 text-foreground/40 dark:text-muted-foreground">
                                {line.lineNumber}
                              </span>
                    ...
                    {viewMode === "diff" && !compareRevision && (
                      <div className="bg-accent rounded-xl p-8 text-center">
                        <GitCompare className="w-12 h-12 text-foreground/30 dark:text-muted-foreground/30 mx-auto mb-3" />
              ...
              <div className="flex items-center justify-center h-[500px]">
                <div className="text-center">
                  <XCircle className="w-12 h-12 text-foreground/30 dark:text-muted-foreground/30 mx-auto mb-3" />
                  <p className="font-[Manrope] text-foreground/60 dark:text-muted-foreground">
                    Bir sürüm seçin
                  </p>
                </div>
