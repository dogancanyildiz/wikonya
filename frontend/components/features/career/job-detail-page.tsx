...
export function JobDetailPage() {
  ...
  const job = {
    ...
    postedDays: 2,
    salary: "5.000₺",
    description: "TechHub Konya'da pazarlama ekibimize katılacak, sosyal medya yönetimi, içerik üretimi ve etkinlik organizasyonu konularında deneyim kazanacak bir stajyer arıyoruz.",
    requirements: [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <CardContent className="p-4 sm:p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-start gap-4 flex-1">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-accent rounded-xl flex items-center justify-center border-2 border-border">
                    <span className="font-[Manrope] text-primary font-extrabold text-2xl sm:text-3xl">
                      {job.companyLogo}
                    </span>
                  </div>
                  <div className="flex-1">
                    <h1 className="font-[Manrope] text-foreground mb-2 font-extrabold text-2xl sm:text-3xl lg:text-4xl">
                      {job.role}
                    </h1>
                    <p className="font-[Manrope] text-foreground/70 dark:text-muted-foreground font-semibold text-base sm:text-lg mb-3">
                      {job.company}
                    </p>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <Badge className={`font-[Manrope] font-bold text-sm px-3 py-1.5 ${typeColors[job.type]}`}>
                  {job.type}
                </Badge>
              </div>
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-2">
                {job.responsibilities.map((responsibility, index) => (
                  <div key={index} className="flex items-start gap-3 p-3 bg-accent rounded-xl">
        ...
        <div className="space-y-4 sm:space-y-6">
            ...
            <CardContent className="space-y-4 pt-0">
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 bg-accent rounded-xl">
                  ...
                  <Badge className={`font-[Manrope] font-bold text-xs ${typeColors[job.type]}`}>
                    {job.type}
                  </Badge>
                </div>
                {job.salary && (
          ...
          <Card className="bg-card rounded-xl shadow-md dark:shadow-lg border border-border">
            ...
            <CardContent className="space-y-4 pt-0">
              <div>
                <p className="font-[Manrope] font-bold text-base text-foreground mb-2">
                  {job.companyInfo.name}
                </p>
                <p className="font-[Manrope] text-sm text-foreground/70 dark:text-muted-foreground leading-relaxed">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-[Manrope] font-semibold text-xs text-foreground">
              ...
              <div className="space-y-2">
                {job.companyInfo.website && (
                  <a
                    ...
                    className="flex items-center gap-2 p-2 bg-accent rounded-lg hover:bg-primary/10 dark:hover:bg-primary/20 transition-colors group"
                  >
                    <Globe className="w-4 h-4 text-primary group-hover:scale-110 transition-transform" />
                    <span className="font-[Manrope] text-sm text-primary group-hover:underline">
                      Web Sitesi
                    </span>
                  </a>
                )}
