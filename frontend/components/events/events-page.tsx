  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-8 lg:px-16 py-4 sm:py-6 md:py-8">
      ...
      <div className="mb-6 flex items-center gap-2 flex-wrap">
        <Filter className="w-4 h-4 text-[#4d4d4d]/60 dark:text-muted-foreground" />
          ...
          return (
            ...
            >
              ...
              <CardContent className="p-4 sm:p-5">
                <Link href={`/events/${event.id}`}>
                  <h3 className="font-[Manrope] text-[#4d4d4d] dark:text-foreground mb-3 group-hover:text-[#03624c] transition-colors font-bold text-lg sm:text-xl leading-snug">
                    {event.title}
                  </h3>
                </Link>

                {event.description && (
                  <p className="font-[Manrope] text-[#4d4d4d]/70 dark:text-muted-foreground text-sm mb-4 line-clamp-2">
                    {event.description}
                  </p>
                )}
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-[Manrope] text-[#4d4d4d]/70 dark:text-muted-foreground font-medium text-sm">
                      {event.date}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  {event.organizer && (
                    <div className="flex items-center gap-2">
                      <Users className="w-4 h-4 text-[#03624c]" />
                      <span className="font-[Manrope] text-[#4d4d4d]/70 dark:text-muted-foreground font-medium text-sm">
                        {event.organizer}
                      </span>
                    </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-[Manrope] text-[#4d4d4d] dark:text-foreground font-semibold text-sm">
                      Katılımcı: {event.participants}/{event.maxParticipants}
                    </span>
                    <span className={`font-[Manrope] font-bold text-xs ${
                      isFull ? 'text-red-500' : 'text-[#03624c]'
                    }`}>
                  ...
                  <div className="w-full h-2 bg-gray-100 dark:bg-accent rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-500 ${
                        isFull 
                          ? 'bg-red-500' 
                          : 'bg-gradient-to-r from-[#03624c] to-[#03624c]/80'
                      }`}
                ...
                <Link href={`/events/${event.id}`}>
                  <Button 
                    className="w-full py-2.5 bg-[#03624c] text-white rounded-xl font-[Manrope] hover:bg-[#03624c]/90 transition-all flex items-center justify-center gap-2 font-bold text-sm"
                    disabled={isFull}
                  >
