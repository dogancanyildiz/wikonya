  ...
  return (
    <Card className="bg-white dark:bg-card rounded-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.06)] dark:shadow-lg border border-border">
      ...
      <CardContent className="space-y-3 sm:space-y-4">
        {events.map((event) => {
          ...
          return (
            <div
              key={event.id}
              className="group rounded-2xl overflow-hidden border-2 border-gray-100 dark:border-border hover:border-[#03624c] transition-all"
            >
              {/* Event Image */}
              ...
              <div className="p-3 sm:p-4">
                <h3 className="font-[Manrope] text-[#4d4d4d] dark:text-foreground mb-2 sm:mb-3 group-hover:text-[#03624c] transition-colors font-bold text-sm sm:text-base leading-snug">
                  {event.title}
                </h3>

                <div className="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4">
                  <div className="flex items-center gap-2">
                    <Calendar className="w-3 h-3 sm:w-4 sm:h-4 text-[#03624c]" strokeWidth={2.5} />
                ...
                <div className="mb-3 sm:mb-4">
                  <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-[Manrope] text-[#4d4d4d] dark:text-foreground font-semibold text-xs sm:text-[13px]">
                        {event.participants}/{event.maxParticipants} Katılımcı
                      </span>
                    </div>
                    <span className="font-[Manrope] text-[#03624c] font-bold text-[10px] sm:text-xs">
                      {Math.round(filledPercentage)}%
                    </span>
                  </div>
                  <div className="w-full h-1.5 sm:h-2 bg-gray-100 dark:bg-accent rounded-full overflow-hidden">
                    <div 
                      className="h-full bg-gradient-to-r from-[#03624c] to-[#03624c]/80 rounded-full transition-all duration-500"
      ...
      <CardContent className="pt-0">
        <Button 
          variant="outline"
          className="w-full py-2 sm:py-3 border-2 border-dashed border-[#03624c] rounded-xl font-[Manrope] text-[#03624c] hover:bg-[#03624c]/5 dark:hover:bg-[#03624c]/10 transition-all font-bold text-xs sm:text-sm"
        >
