    ...
    <div className="mb-6 sm:mb-8">
      ...
      <DropdownMenu modal={false}>
        <DropdownMenuTrigger asChild>
          <Button 
            variant="outline"
            className="px-4 sm:px-5 py-2 sm:py-3 bg-white dark:bg-card text-[#4d4d4d] dark:text-foreground rounded-2xl font-[Manrope] hover:bg-[#f2f4f3] dark:hover:bg-accent transition-colors shadow-sm font-bold text-xs sm:text-sm border border-border flex items-center gap-2"
          >
            {selectedFilter ? (
              <>
                {selectedFilter.icon && (
          ...
          {filters.map((filter) => {
            ...
            return (
              <DropdownMenuItem
                ...
                className={isActive ? 'bg-[#03624c] text-white' : ''}
              >
