      ...
      <CardContent className="p-6 sm:p-10 flex flex-col flex-1">
        <div className="flex items-start gap-4 sm:gap-6 flex-1">
          ...
          <div className="flex-1 flex flex-col">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
              ...
              <div className="px-3 sm:px-4 py-1 bg-primary rounded-full">
                <span className="font-[Manrope] text-primary-foreground font-bold text-xs sm:text-sm">
            ...
            {user.bio && (
              <p className="font-[Manrope] text-foreground leading-relaxed font-medium text-xs sm:text-sm flex-1">
                {user.bio}
              </p>
            )}
          </div>
        ...
        <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-border">
          ...
          <div className="mb-4 sm:mb-6">
            <div className="flex items-center justify-between mb-2 sm:mb-3">
              <div className="flex items-center gap-2">
                <div className="bg-gradient-to-br from-primary to-primary/80 rounded-xl p-2 sm:p-2.5 text-primary-foreground">
                  <Gift className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
                <div>
                  <p className="font-[Manrope] font-bold text-xs sm:text-sm text-foreground">
                    {roleInfo.description.toUpperCase()}
                  </p>
                  <p className="font-[Manrope] opacity-90 font-medium text-[10px] sm:text-xs text-muted-foreground">
                    {ROLE_DISPLAY_NAMES[user.role].toUpperCase()}
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p className="font-[Manrope] font-bold text-xs sm:text-sm text-primary">
                  {xpInfo.current.toLocaleString()} / {xpInfo.nextLevel.toLocaleString()} XP
                </p>
                <p className="font-[Manrope] font-medium text-[10px] sm:text-xs text-muted-foreground">
            ...
            {/* XP Bar */}
            <div className="w-full bg-accent rounded-full h-3 sm:h-4 overflow-hidden">
              <div 
            ...
            <div className="flex items-center justify-between mt-1.5 sm:mt-2">
              <span className="font-[Manrope] font-semibold text-[9px] sm:text-[10px] text-muted-foreground/70">
                {roleInfo.minCoins.toLocaleString()} XP
              </span>
              {coinsNeeded !== null ? (
                <span className="font-[Manrope] font-semibold text-[9px] sm:text-[10px] text-primary">
          ...
          <div className="flex items-center gap-2 mb-2 sm:mb-3">
            ...
            <h3 className="font-[Manrope] text-foreground font-bold text-sm sm:text-base">
            ...
            {user.badges.length > 0 ? (
              <>
                {user.badges.slice(0, 3).map((badge) => (
                  <div
                    key={badge.id}
                    className="flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-primary to-primary/80 rounded-xl"
                    title={badge.name}
                  >
                    <span className="font-[Manrope] text-primary-foreground font-extrabold text-lg sm:text-2xl lg:text-[24px]">
                      {badge.icon}
                    </span>
                  </div>
                ))}
                {user.badges.length > 3 && (
                  <div className="flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-accent rounded-xl">
                    <span className="font-[Manrope] text-muted-foreground/60 font-bold text-xs sm:text-xs">
                      +{user.badges.length - 3}
                    </span>
                  </div>
                )}
              ...
              <p className="font-[Manrope] text-muted-foreground text-xs sm:text-sm">
