...
export function MarkdownEditor({
  ...
  const renderMarkdown = (text: string) => {
    ...
    let html = text
      ...
      // Linkler
      .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-[#03624c] hover:underline" target="_blank" rel="noopener noreferrer">$1</a>')
      // Liste
      .replace(/^- (.*$)/gim, "<li class='ml-4'>$1</li>")
      // Alıntı
      .replace(/^> (.*$)/gim, "<blockquote class='border-l-4 border-[#03624c] pl-4 italic my-2'>$1</blockquote>")
      // Paragraflar
      .split("\n")
      .map((line) => {
  ...
  return (
    <div className="space-y-2">
      {/* Toolbar */}
      <div className="flex items-center gap-1 p-2 bg-[#f2f4f3] dark:bg-accent rounded-lg border border-border">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "edit" | "preview")} className="w-full">
        ...
        <TabsContent value="preview" className="mt-2">
          <ScrollArea className={`border rounded-lg p-4 bg-white dark:bg-card ${minHeight ? `min-h-[${minHeight}]` : ""}`} style={{ minHeight }}>
            {value ? (
              <div
                className="prose prose-sm max-w-none font-[Manrope] text-[#4d4d4d] dark:text-foreground"
